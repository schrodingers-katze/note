...
```c++
// 기존 코드 : down() 함수를 한 번씩 실행하며 한 칸씩 아래로 이동하였습니다. 예를 들어, 두 칸 움직이고 싶을 경우, 두 번 호출합니다.
void down(vector<pair<int, int> > vp) {
...
```c++
// 개선 후의 코드 : down() 함수를 한 번 실행하여 더 이상 아래로 내릴 수 없을 만큼 이동합니다. 예를 들어, 두 칸 움직이면 바닥에 닿을 경우라도, 한 번만 함수를 호출합니다.
void down(vector<pair<int, int> > vp) {
